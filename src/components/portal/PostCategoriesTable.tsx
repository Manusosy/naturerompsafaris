...
export function PostCategoriesTable({
  ...
  async function handleBulkApply() {
    ...
    if (!window.confirm(`Delete ${selectedIds.length} categor${selectedIds.length === 1 ? "y" : "ies"}?`)) return;

    setBusyId("bulk");
    await Promise.all(
      selectedIds.map((id) =>
        fetch("/api/portal/records", {
          body: JSON.stringify({ action: "delete", collection: "post-categories", id }),
          credentials: "include",
              ...
              return (
                <tr className="wp-row-hover" key={id}>
                  ...
                  <td className="taxonomy-table__name">
                    <Link href={`/admin/post-categories/${id}`}>{String(doc.name ?? "Category")}</Link>
                    <div className="row-actions">
                      <Link href={`/admin/post-categories/${id}`}>Edit</Link>
                      {" | "}
                  ...
                  <td className="taxonomy-table__count">
                    <Link href={`/admin/posts?category=${id}`}>{postCounts[id] || 0}</Link>
                  </td>
                </tr>
              );
            })}
